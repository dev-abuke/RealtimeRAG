...
class RAGPipeline:
    ...
    def query(
        ...
        self.freshness_scorer = FinancialFreshnessScorer()
        ...
        try:
            ...
            for article in retrieved_results:
                ...
                boost = self.freshness_scorer.get_boost_factor(freshness['confidence_score'])
                article.score = article.score * boost  # Adjust the score

            # Sort articles by adjusted score
            retrieved_results.sort(key=lambda a: a.score, reverse=True)
