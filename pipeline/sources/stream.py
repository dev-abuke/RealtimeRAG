...
class AlpacaNewsStreamInput(DynamicSource):
    ...
    def build(self, step_id, worker_index, worker_count):
        logger.info(f"Building Alpaca news stream input... step_id: {step_id}")
        """
...
class AlpacaNewsStreamSource(StatelessSourcePartition):
    """
...
def build_alpaca_client(
    ...
    if api_key is None:
        try:
            api_key = constants.ALPACA_NEWS_API_KEY
        except KeyError:
            raise KeyError(
    ...
    if api_secret is None:
        try:
            api_secret = constants.APACA_NEWS_SECRET
        except KeyError:
            raise KeyError(
